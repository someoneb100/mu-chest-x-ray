...
def get_images(directory: str):
    for root, _, files in walk(directory):
        for file in files:
            file_path = path.relpath(path.join(root,file), start = directory)
            is_pneumonia = root.endswith("PNEUMONIA")
            yield (file_path, is_pneumonia)

def normalize_directory(src_dir: str, dest_dir: str, new_shape: "tuple[int, int]") -> None:
    for img_name, is_pneumonia in get_images(src_dir):
        src_img = Image.open(path.join(src_dir, img_name))
        dest_img = normalize_image(src_img, new_shape)
        dest_path = path.join(dest_dir, str(is_pneumonia))
        if not path.isdir(dest_path):
            makedirs(dest_path)
        dest_img.save(path.join(dest_path, path.split(img_name)[-1]))
